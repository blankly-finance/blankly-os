...
export default function EditLabelModal(props: { label: string, open: boolean, update: Function, close: Function }) {
  const formik = useFormik({
    initialValues: {
      label: props.label,
    },
    onSubmit: (values) => {
      props.update(values.label);
      closeModal();
    },
  });
  useEffect(() => {
    formik.setValues({label: props.label})
              ...
              <div>
                <div className="mt-3 text-center sm:mt-5">
                  <Dialog.Title as="h3" className="text-lg leading-6 font-medium text-gray-900">
                    Add a Label for Your Backtest
                  </Dialog.Title>
                  <div className="mt-4">
                    <p className="text-sm text-gray-500">
                      This helps you keep track of what you tested
                    </p>
                  </div>
                </div>
              </div>
