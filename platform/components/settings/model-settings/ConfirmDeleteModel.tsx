...
const ConfirmDeleteModel = (props: { close: any, open: boolean }) => {
  ...
  const deleteModel = useCallback(() => {
    setDeleting(true)
    deleteDoc(`projects/${projectId}/models/${modelId}`).then(function (response) {
      router.push(`/${projectId}`);
    })
  }, [modelId, projectId, router])

  useEffect(() => {
    getModelOnce(projectId as string, modelId as string).then((doc: any) => {
          ...
          >
            <div
              ...
              <div>
                ...
                <div className="mt-3 text-center sm:mt-5">
                  <Dialog.Title as="h3" className="text-lg leading-6 font-medium text-gray-900">
                    {deleting ? "Deleting Model" : "Delete Model"}
                  </Dialog.Title>
                  <div className="mt-2">
                    {deleting ? <p className="text-gray-600 w-4/5 mx-auto">
                      Deleting this model. Please do not leave this page.
                    </p> : <p className="text-gray-600 w-4/5 mx-auto">
                      Are you sure you want to delete this model? This will delete any records, backtests, versions
                      associated with this model. This action cannot be reversed. If you wish to continue, please type
                      the name of your model below.
                    </p>}
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 flex-col flex space-y-2 px-10">
                <button
                  disabled={confirm !== modelName && !deleting}
                  type="button"
                  className={classNames("w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 text-base font-medium text-white", confirm == modelName && !deleting ? "bg-red-600 hover:bg-red-700" : "cursor-not-allowed bg-red-300")}
                  onClick={() => deleteModel()}
                >
                  {
                    deleting ? (
