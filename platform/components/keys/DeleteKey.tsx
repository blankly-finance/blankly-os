...
import {deleteKey} from '@/services/deployment-api-store'

export default function DeleteKey({ projectId, keyId, open, close }: any) {
    const { token } = useAuth();
    const cancelButtonRef = useRef(null)

    const removeKey = useCallback(() => {
        deleteKey(token, projectId, keyId)
        close(true)
    }, [close, keyId, projectId, token])
                            ...
                            <div>
                                ...
                                <div className="mt-3 text-center sm:mt-5">
                                    <Dialog.Title as="h3" className="text-lg leading-6 font-medium text-gray-900">
                                        Are you sure you want to delete this key {keyId}?
                                    </Dialog.Title>
                                    <div className="mt-2">
                                        <p className="text-sm text-gray-500">
                                            This action is not reversible, and the models associated with this key will not work
                                        </p>
                                    </div>
                                </div>
                            </div>
